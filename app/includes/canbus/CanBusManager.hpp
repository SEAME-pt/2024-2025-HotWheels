...
class CanBusManager : public QObject
{
    ...
    Q_OBJECT
public:
    /** @brief Constructor for the CanBusManager class that takes a SPI device path. */
    explicit CanBusManager(const std::string &spi_device, QObject *parent = nullptr);
    /** @brief Constructor for the CanBusManager class that takes a MCP2515 controller. */
...
private:
    ...
    /** @brief Flag to indicate if the MCP2515 controller is owned by the CanBusManager. */
