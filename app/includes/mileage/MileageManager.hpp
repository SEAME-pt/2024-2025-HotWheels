...
public:
     ...
     * Initializes the class with a file path for mileage data and sets up the file handler.
     *
...
public slots:
     ...
     * It calculates the total distance for the last interval and adds it to the total mileage.
     */
    void updateMileage();
...
private:
    MileageCalculator calculator; /**< A calculator that computes the distance based on speed and time intervals. */
    MileageFileHandler fileHandler; /**< A file handler to read and write mileage data. */
    QTimer updateTimer; /**< A timer that triggers mileage updates every 1 second. */
    QTimer persistenceTimer; /**< A timer that triggers saving mileage every 10 seconds. */
