 ...
 * @date 2025-01-30
 * 
 * @details This file contains unit tests for the CanBusManager class, using Google Test and Google Mock frameworks.
 * 
 ...
 * 
 * @details This test uses `QSignalSpy` to verify that `speedUpdated` emits the expected
 *          speed value when the mock controller triggers the signal.
 * 
 ...
 * @brief Ensures that `initialize()` fails when the controller fails to initialize.
 * 
 * @details This test sets up a mock expectation that `init()` will return `false`,
 *          verifying that `initialize()` correctly fails in this case.
 * 
 ...
 * @brief Ensures that `initialize()` succeeds when the controller initializes successfully.
 * 
 * @details This test sets up a mock expectation that `init()` will return `true`,
 *          verifying that `initialize()` correctly returns `true`.
 * 
 * @see CanBusManager::initialize
 */
...
/**
 * @test Ensures that `stopReading()` is called when `CanBusManager` is destroyed.
 ...
 * 
 * @note This test indirectly verifies proper cleanup behavior in the destructor.
 * 
