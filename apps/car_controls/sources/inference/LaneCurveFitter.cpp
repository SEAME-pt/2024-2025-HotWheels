...
std::pair<LaneCurveFitter::LaneCurve *, LaneCurveFitter::LaneCurve *> LaneCurveFitter::selectRelevantLanes(
    std::vector<LaneCurveFitter::LaneCurve> &lanes, int imgWidth, int imgHeight)
{
    ...
    std::vector<LaneCurve> &lanes, int imgWidth, int imgHeight)
{
    ...
    if (leftLane && rightLane)
    {
        ...
        std::cout << "Using both lanes: left and right with "
                  << leftLane->curve.size() << " and "
                  << rightLane->curve.size() << " points respectively." << std::endl;
    ...
    else if (leftLane || rightLane)
    {
        ...
        std::cout << "Using single lane: "
                  << (leftLane ? "left" : "right") << " lane with "
                  << lane->curve.size() << " points." << std::endl;
    }
