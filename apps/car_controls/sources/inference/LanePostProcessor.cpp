...
cv::cuda::GpuMat LanePostProcessor::process(const cv::cuda::GpuMat& rawMaskGpu) {
    ...
    cv::cuda::threshold(rawMaskGpu, binaryMaskGpu, 0.35, 255.0, cv::THRESH_BINARY);
    binaryMaskGpu.convertTo(binaryMaskGpu, CV_8U);

    cv::Mat binaryMask;
    binaryMaskGpu.download(binaryMask); 

    std::vector<LaneInfo> initialLanes = extractLaneInfo(binaryMask, false, true);

	// // std::cout << "Initial lanes extracted: " << initialLanes.size() << std::endl;
    ...
    // std::vector<LaneInfo> finalLanes = extractLaneInfo(binaryMask, true, false);
    std::vector<LaneInfo> finalLanes = extractLaneInfo(grayMerged, true, false);

	// std::cout << "Final lanes after filtering: " << finalLanes.size() << std::endl;
