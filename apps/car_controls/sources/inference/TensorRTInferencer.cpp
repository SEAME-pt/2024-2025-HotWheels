...
TensorRTInferencer::TensorRTInferencer(const std::string &enginePath) : runtime(nullptr),		// Initialize runtime pointer to nullptr
																		...
																		hostOutput(nullptr)
{ // Initialize host output pointer to nullptr
	...
	if (!runtime)
	{ // Check if runtime creation failed
	...
	{ // Verify both input and output were found
	...
	if (inputDims.d[0] == -1)
	{																		   // If input has dynamic batch dimension
	...
	{ // Check if any output dimension is dynamic
		if (outputDims.d[i] < 0)
		{
	...
	for (int i = 0; i < outputDims.nbDims; i++)
	{ // Multiply all output dimensions
	...
	if (status != cudaSuccess)
	{						   // Check output memory allocation
...
{
	if (deviceInput)
		cudaFree(deviceInput); // Free input buffer if allocated
	if (deviceOutput)
		cudaFree(deviceOutput); // Free output buffer if allocated
	if (stream)
		cudaStreamDestroy(stream); // Destroy CUDA stream if created
	if (lanePostProcessor)
		delete lanePostProcessor; // Delete post-processor object
	if (laneCurveFitter)
		delete laneCurveFitter; // Delete post-processor and curve fitter objects
...
{
	if (hostInput)
		cudaFreeHost(hostInput); // Free pinned host memory for input
	if (hostOutput)
		cudaFreeHost(hostOutput); // Free pinned host memory for output
...
cv::cuda::GpuMat TensorRTInferencer::preprocessImage(const cv::cuda::GpuMat &gpuImage)
{
	...
	if (gpuImage.type() != CV_8UC3 && gpuImage.type() != CV_8UC1)
	{ // Check if input image is in expected format
	...
	{															   // If input has multiple channels (color)
...
void TensorRTInferencer::initUndistortMaps()
{
	...
	cv::initUndistortRectifyMap(
		...
		cv::Size(1280, 720),
		CV_32FC1, mapx, mapy); // Compute undistortion mapping
...
void TensorRTInferencer::doInference(const cv::Mat &frame)
{
	...
	// Display the centerline result
	if (centerlineResult.valid)
	{
		// Draw the centerline on the post-processed mask
		cv::Mat maskCenterline;
		d_postprocessed.download(maskCenterline); // Download post-processed mask to CPU

		const auto &centerline = centerlineResult.blended;
		for (size_t i = 1; i < centerline.size(); ++i)
		{
			const auto &pt1 = centerline[i - 1];
			const auto &pt2 = centerline[i];
			// cv::line(maskCenterline, pt1, pt2, cv::Scalar(0, 0, 255), 2);
			cv::line(maskCenterline, cv::Point2f(pt1.x, pt1.y), cv::Point2f(pt2.x, pt2.y), cv::Scalar(0, 0, 255), 2);
		}

		// Re-upload the mask with centerline to GPU
		cv::cuda::GpuMat d_maskCenterline;
		d_maskCenterline.upload(maskCenterline);
		
		Publisher::instance(INFERENCE_PORT)->publishInferenceFrame(INFERENCE_TOPIC, d_maskCenterline); // Publish frame to ZeroMQ publisher
	} else {
